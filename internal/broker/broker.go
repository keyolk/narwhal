...
func ParseSplitRequest(content string) (taskID, name, assignment string, deps []string, ok bool) {
	...
	// The assignment is free text a worker wrote and routinely contains
	// pipes — shell pipelines, tables, alternations. Splitting left to
	// right gave it a fixed number of fields and handed the overflow to
	// deps: on run s1786800188852-1 the tail of a `tar -c . | tar -x` line
	// became task-5's dependency list, and the task sat pending across four
	// restarts waiting on names no task could ever have.
	//
	// deps are task ids, which cannot contain a pipe, so the last field is
	// found from the right and everything between belongs to the
	// assignment. This reads the messages already on disk unchanged.
	parts := strings.SplitN(rest[1:], "|", 3)
	if len(parts) < 3 {
		return "", "", "", nil, false
	}
	...
	assignment = parts[2]
	depField := ""
	if i := strings.LastIndexByte(assignment, '|'); i >= 0 {
		depField = assignment[i+1:]
		assignment = assignment[:i]
	}
	for _, d := range strings.Split(depField, ",") {
		d = strings.TrimSpace(d)
		// A dep that cannot be a task id is not a dependency that has yet
		// to be created — it is text that leaked out of another field, and
		// keeping it makes the task permanently unreachable because
		// recomputeReady returns early on a dep it cannot find.
		if d == "" || !validTaskID(d) {
			continue
		}
		deps = append(deps, d)
...
// validTaskID reports whether s could be a task id. Ids are generated by
// the planner and by workers calling `split`, so they are conservative:
